...
def test_doc_count_one_mapper(tmpdir):
    ...
    """
    shutil.copy("hadoop/inverted_index/map0.py", tmpdir)
    shutil.copy("hadoop/inverted_index/reduce0.py", tmpdir)
...
def test_simple(tmpdir):
    ...
    https://docs.pytest.org/en/6.2.x/tmpdir.html#the-tmpdir-fixture
    """
    utils.copyglob("hadoop/inverted_index/map?.py", tmpdir)
    utils.copyglob("hadoop/inverted_index/reduce?.py", tmpdir)
    utils.copyglob("hadoop/inverted_index/stopwords.txt", tmpdir)
...
def test_sample_inverted_index(tmp_path):
    ...
    made using hadoop/inverted-index/input/input.csv
    ...
    # Verify inverted index file are copied to ./index/index/inverted_index
    inverted_index_dir = Path("index/index/inverted_index")
